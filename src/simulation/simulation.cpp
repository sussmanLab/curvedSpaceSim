...
void simulation::computeMonodisperseStress(vector<double>& stress)
    ...
    int Ndof = conf->N;
    double density
        = Ndof / meshSurfaceArea; // using number/area density definition to avoid mismatch b/w differing cutoffs of force computers
    ...
    for (unsigned int f = 0; f < forceComputers.size(); ++f)
        ...
        for (int ii = 0; ii < Ndof; ++ii)
            ...
            for (int jj = 0; jj < neighborNumber; ++jj)
                {
                vector3 separation
                    = conf->neighborVectors[ii][jj]; // specifies the separation vector between particle i and its jth neighbor
    ...
    // sigma = (1/2) rho kb <(vi outer vi)> + <f_ij outer dr_ij>/(2*d*V), d = 2 -- dimension here is NOT the same as above, as now we are
    // restricting to surface
