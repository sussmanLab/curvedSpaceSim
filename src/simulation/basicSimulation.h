...
//! Only child classes are actually used in a main cpp file
...
public:
    ...
    //! manipulate data from updaters...on multirank simulations, might be needed to get, e.g., current total KE, or the current size of the
    //! force vector
    ...
    virtual void reportSelf()
        {
        cout << "in the base simulation class" << endl;
        cout.flush();
        };
