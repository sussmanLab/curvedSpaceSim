...
void mpiSimulation::synchronizeAndTransferBuffers()
    ...
    if (totalRanks > 1)
        ...
        MPI_Allgather(&(Conf->intTransferBufferSend)[0], n, MPI_INT, &(Conf->intTransferBufferReceive)[0], n, MPI_INT, MPI_COMM_WORLD);
        MPI_Allgather(
            &(Conf->doubleTransferBufferSend[0]), 3 * n, MPI_DOUBLE, &(Conf->doubleTransferBufferReceive[0]), 3 * n, MPI_DOUBLE,
            MPI_COMM_WORLD
        );
...
*/
void mpiSimulation::setConfiguration(MPIConfigPtr _config) { mConfiguration = _config; };

/*!
For now: in your main cpp file make a database class on rank 0, and use it (and
its global information) to save the state.
*/
void mpiSimulation::saveState(string fname) { UNWRITTENCODE("saving states in a mpi simulation not written"); };
