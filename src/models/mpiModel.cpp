...
void mpiModel::findNeighbors(double maximumInteractionRange)
    ...
    for (int ii = 0; ii < N; ++ii)
        {
        // use the neighborStructure to find candidate neighbors. This function will fill the indices of the neighbors[ii] data structure
        // and populate the positions of the corresponding targetParticles. minIdx provides the necessary offset for indexing between
        // positions and globalPositions
        double largestNeighborDistance;
        vector<meshPosition> targetParticles;
        // if needed, target a euclidean position
        if (euclideanNeighborsMeshPositions)
            largestNeighborDistance = neighborStructure->constructCandidateNeighborList(
                euclideanMeshPosition[ii + minIdx], ii, neighbors[ii], targetParticles, minIdx
            );
        else
...
void mpiModel::broadcastParticleVelocities(vector<vector3>& v, int broadcastRoot)
    ...
    MPI_Bcast(&doubleTransferBufferReceive[0], 3 * NTotal, MPI_DOUBLE, broadcastRoot, MPI_COMM_WORLD);
    //...and read in the appropriate part. This could be simplified (as right now all ranks get the full vector of velocities in the
    //buffers)
    for (int ii = 0; ii < N; ++ii)
        {
        double vx, vy, vz;
...
void mpiModel::fillEuclideanLocations() { space->meshPositionToEuclideanLocation(globalPositions, euclideanLocations); };
