...
void triangulatedMeshSpace::updateMeshSpanAndTree()
    ...
    minVertexPosition.x = 0;
    minVertexPosition.y = 0;
    minVertexPosition.z = 0;
    maxVertexPosition.x = 0;
    maxVertexPosition.y = 0;
    maxVertexPosition.z = 0;
    ...
    if (verbose)
        printf(
            "mesh spans (%f,%f,%f) to (%f,%f,%f)\n", minVertexPosition.x, minVertexPosition.y, minVertexPosition.z, maxVertexPosition.x,
...
double triangulatedMeshSpace::getArea() { return totalArea(surface); }

void triangulatedMeshSpace::distanceWithSubmeshing(
    meshPosition& p1, std::vector<meshPosition>& p2, std::vector<double>& distances, std::vector<vector3>& startPathTangent,
    std::vector<vector3>& endPathTangent, double distanceThreshold
)
    {
    // create submesh:
    ...
    triangleMesh submesh = submeshAssistant.constructSubmeshFromSourceAndTargets(
        surface, sourcePoint, faceTargetsForSubmesh, currentDistanceThreshold, vertexMap, faceMap
    );
...
*/
void triangulatedMeshSpace::distance(
    meshPosition& p1, std::vector<meshPosition>& p2, std::vector<double>& distances, std::vector<vector3>& startPathTangent,
    std::vector<vector3>& endPathTangent, double distanceThreshold
)
    {
...
*/
std::pair<faceIndex, vector3>
triangulatedMeshSpace::throughVertex(vertexIndex& intersectedVertex, vector3& toIntersection, faceIndex& sourceFace)
    {
    ...
    for (halfedgeIndex hi : halfedges_around_face(hf, surface))
        {
        if (source(hi, surface) == intersectedVertex)
            startingVertex = target(hi, surface);
        }
    ...
    while (unfinished)
        ...
        if (started && atStartPoint)
            unfinished = false; // finishing counting, or
    ...
    while (angleTraveled < angleToTravel)
        ...
        vector3 e2 = edgeVectors[(counter + 1) % numNeighbors]; // leaving this here for instances where one face is gigantic and the others
                                                                // are miniscule
...
// The following are special helpers for transportParticleAndVelocity, designed to help manage source points staying within faces and
// perform projections at boundaries.
...
void triangulatedMeshSpace::transportParticleAndVectors(meshPosition& pos, vector3& displacementVector, vector<vector3>& transportVectors)
    ...
    while (continueShifting)
        ...
        // if the targetBarycentricLocation is in the face, we have found our destination, so check this before implementing all of the
        // intersection locating and vector rotating logic
        ...
        findTriangleEdgeIntersectionInformation(
            sourceBarycentricLocation, targetBarycentricLocation, intersectionPoint, vertexList, lastUsedHalfedge, surface, involvedVertex,
            uninvolvedVertex
        );
        ...
        if (uninvolvedVertex.size() == 2)
            ...
            if (surface.is_border(vertexIndex(involvedVertex[0])))
                ...
                updateForVertexIntersection(
                    sourceBarycentricLocation, sourcePoint, currentSourceFace, target, displacementVector, currentSourceNormal,
                    vertexPositions, transportVectors, lastUsedHalfedge, involvedVertex[0], toIntersection
                );
                continue;
                }
            }
            ...
            {
            printSourceTargetDisplacementInfo(
                sourcePoint, target, sourceBarycentricLocation, targetBarycentricLocation, displacementVector
            );
            ERRORERROR("a barycentric coordinate of the target is negative, but neither 1 nor 2 intersections were found. Apparently some "
                       "debugging is needed!");
            }
            ...
            if (surface.is_border(edgeIndex(intersectedEdge)))
                ...
                updateForEdgeIntersection(
                    sourceBarycentricLocation, sourcePoint, intersectionPoint, currentSourceNormal, currentSourceFace, target,
                    transportVectors, lastUsedHalfedge, displacementVector, vertexPositions, intersectedEdge
                );
...
void triangulatedMeshSpace::updateForVertexIntersection(
    pmpBarycentricCoordinates& sourceBCs, point3& sourcePoint, faceIndex& sourceFace, point3& target, vector3& displacementVector,
    vector3& sourceNormal, vector<point3> vertexPositions, vector<vector3>& transportVectors, halfedgeIndex& lastUsedHalfedge,
    vertexIndex intersectedV, vector3 toIntersection
)
    {
...
void triangulatedMeshSpace::updateForEdgeIntersection(
    pmpBarycentricCoordinates& sourceBarycentricLocation, point3& sourcePoint, pmpBarycentricCoordinates& intersectionPoint,
    vector3& currentSourceNormal, faceIndex& currentSourceFace, point3& target, vector<vector3>& transportVectors,
    halfedgeIndex& lastUsedHalfedge, vector3& displacementVector, vector<point3>& vertexPositions, halfedgeIndex intersectedEdge
)
    {
...
void triangulatedMeshSpace::printSourceTargetDisplacementInfo(
    point3 sourcePoint, point3 target, pmpBarycentricCoordinates sourceBarycentricLocation,
    pmpBarycentricCoordinates targetBarycentricLocation, vector3 displacementVector
)
    {
