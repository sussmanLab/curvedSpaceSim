...
public:
    ...
    //! Given a particle somewhere on the mesh, displace it in the direction of the vector, wrapping around faces; any vectors given to the
    //! function are transported along with it
    virtual void transportParticleAndVectors(meshPosition& pos, vector3& displacementVector, vector<vector3>& transportVectors);

    //! Given a source particle and a vector of target points, determine the geodesic distance and store the start and end path tangents
    //! along the paths
    virtual void distance(
        ...
        std::vector<vector3>& endPathTangent, double distanceThreshold = VERYLARGEDOUBLE
    );
    ...
    void setNewSubmeshCutoff(double newCutoff) { useSubmeshingRoutines(true, newCutoff); }

    //! given a vector of meshPositions that represent barycentric coordinates, fill a second vector of meshPositions that represent the
    //! corresponding R3 positions
    ...
    virtual void printSourceTargetDisplacementInfo(
...
protected:
    ...
    // data structures associated with the potential for "dangerous" submeshes -- these can occur when submeshing routine is capable of
    // returning a submesh with multiple connected components (e.g., when dealing with a surface that looks like an elephant's ear)
