...
void openMeshSpace::getBoundaryVertexHeading(
    vertexIndex v, faceIndex& sourceFace, vector3& displacementVector, pmpBarycentricCoordinates& sourceBarycentricLocation,
    vector<point3> vertexPositions, vector3 sourceNormal, std::pair<vector3, vertexIndex>& result
)
    {
...
void openMeshSpace::projectVectorsForBoundaryVertex(
    vector3 heading, vertexIndex intersectedV, vertexIndex boundaryV, vector3 fNormal, faceIndex face, vector<vector3>& transportVectors
)
    {
    ...
    while (continueShifting)
        ...
        if (uninvolvedVertex.size() == 2)
            ...
            if (surface.is_border(vertexIndex(involvedVertex[0])))
                updateAtBoundaryVertex(
                    sourceBarycentricLocation, targetBarycentricLocation, target, currentSourceNormal, currentSourceFace,
                    displacementVector, involvedVertex[0], vertexPositions, transportVectors, lastUsedHalfedge, continueShifting
                );
            ...
            if (surface.is_border(edgeIndex(intersectedEdge)))
                updateAtBoundaryEdge(
                    sourceBarycentricLocation, targetBarycentricLocation, target, currentSourceFace, currentSourceNormal, involvedVertex[0],
                    involvedVertex[1], vertexPositions[uninvolvedVertex[0]], transportVectors, displacementVector, lastUsedHalfedge,
                    continueShifting
                );
