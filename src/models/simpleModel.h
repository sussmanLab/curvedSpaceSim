...
Importantly, these models must also implement a moveParticles function in order to interface with the simulation classes, and a
findNeighbors function to interface with force classes Other functions exist to help with initialization and allow for operation in both
euclidean and mesh spaces
*/
class simpleModel
    {
public:
    ...
    virtual void setSpace(shared_ptr<baseSpace> _space) { space = _space; }

    //! Neighbor structures accelerate the process of finding neighbors...if not set, defaults to all-to-all searches
    virtual void setNeighborStructure(shared_ptr<baseNeighborStructure> _structure) { neighborStructure = _structure; }
    ...
    //! takes a vector of euclidean positions, and appends to "simPositions" a set of (barycentric coordinates, face index) of the closest
    //! point on the mesh.
...
protected:
    ...
    //! For compatibility across spaces (e.g., mesh-based spaces), a space may have internal functionality to jam euclidean data into a
    //! meshPosition. This can be used for neighborStructures
