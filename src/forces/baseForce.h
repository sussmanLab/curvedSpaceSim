...
public:
    ...
    virtual string reportSelfName()
        {
        string ans = "base force";
        return ans;
        };
    ...
    //! child classes must implement a specific function that, given a unit vector specifying the direction of separation and scalar
    //! distance, returns an energy
    virtual double pairwiseEnergy(vector3 separation, double distance) = 0;
    //! child classes must implement a specific function that, given a unit vector specifying the direction of separation and scalar
    //! distance, returns a vector force
    ...
    virtual double getClassSize() { return 0.000000001 * ((2 + 0) * sizeof(double)); }
    };
