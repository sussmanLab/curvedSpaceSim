...
space principles to non-Hamiltonian systems" paper (jcp 2001) suggests the correct setting of the first thermostat chain mass to guarantee
conservation of energy in the context of a total momentum=0 setting
*/
...
void noseHooverNVT::propagateChain()
    ...
    for (int ii = chainLength - 1; ii > 0; --ii)
        ...
        bathVariables[ii].z
            = (bathVariables[ii - 1].w * bathVariables[ii - 1].y * bathVariables[ii - 1].y - temperature) / bathVariables[ii].w;
