#include "noInteractionNVT.h"

noInteractionNVT::noInteractionNVT(double _dt, double _T, double _tau, int _M)
    {
    deltaT = _dt;
    dt2 = 0.5*deltaT;
    dt4 = 0.25*deltaT;
...
*/
void noInteractionNVT::setBathVariables()
    {
...
*/
void noInteractionNVT::performUpdate()
    {
...
*/
void noInteractionNVT::propagateChain()
    {
...
Force would usually be required. 
*/
void noInteractionNVT::propagatePositionsVelocities()
    {
    kineticEnergy = 0.0;

    //move particles for the first half of the timestep; no forces to compute, so this could really be replaced
    ...
    for  (int ii = 0; ii < Ndof; ++ii)
        {
        model->velocities[ii] = model->velocities[ii];
        displacements[ii] = dt2*model->velocities[ii];
...
double noInteractionNVT::getTemperatureFromKE()
    {
    double v2sum = 0; 
