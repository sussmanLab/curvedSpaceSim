...
double fireMinimization::updaterVectorDotProduct(vector<vector3>& v1, vector<vector3>& v2)
    ...
    sim->manipulateUpdaterData(dotProd, [](double x, double y) -> double { return x + y; });
    return dotProd[0];
    };
...
void fireMinimization::setFIREParameters(
    int _maximumIterations, double _deltaT, double _alphaStart, double _deltaTMax, double _deltaTMin, double _deltaTInc, double _deltaTDec,
    double _alphaDec, int _nMin, double _forceCutoff, double _alphaMin
)
    {
