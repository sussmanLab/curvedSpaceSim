...
//! return true if the two lines which pass through the given endpoints intersect between the specified points on both lines. fill in the
//! barycentric location of the intersection point
...
//! specialize the intersectionOfLinesInBarycentricCoordinates function to the case where you care about the intersection of "line2" with a
//! "line1" which goes from one vertex to another (i.e., has barycentric coordinates which are a permutation of (1,0,0))
...
void clampAndUpdatePosition(
    pmpBarycentricCoordinates& baryLoc, point3& r3Loc, faceIndex& sFace, triangleMesh& surface, bool belowZero = false
);
