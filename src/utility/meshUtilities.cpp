...
double meanEdgeLength(triangleMesh mesh, bool verbose /*=false*/)
    ...
    if (verbose)
        std::cout << "edge length min, max, mean: " << min << " " << max << " " << mean << "\n";
...
0 <= t2 <= 1
*/
bool intersectionOfLinesInBarycentricCoordinates(
    pmpBarycentricCoordinates line1Start, pmpBarycentricCoordinates line1End, pmpBarycentricCoordinates line2Start,
    pmpBarycentricCoordinates line2End, pmpBarycentricCoordinates& intersectionPoint
)
    {
    double intersectionScale1, intersectionScale2, denominator;
    denominator
        = (line1Start[0] - line1End[0]) * (line2End[1] - line2Start[1]) - (line2End[0] - line2Start[0]) * (line1Start[1] - line1End[1]);
    if (denominator == 0)
        return false;

    intersectionScale1 = (line1Start[0] * (line2End[1] - line2Start[1]) + line2Start[0] * (line1Start[1] - line2End[1])
                          + line2End[0] * (line2Start[1] - line1Start[1]))
                         / denominator;
    intersectionScale2 = (line1Start[0] * (line1End[1] - line2Start[1]) + line1End[0] * (line2Start[1] - line1Start[1])
                          + line2Start[0] * (line1Start[1] - line1End[1]))
                         / denominator;
    ...
    return (intersectionScale1 >= 0 && intersectionScale1 <= 1 && intersectionScale2 >= 0 && intersectionScale2 <= 1);
    };

bool intersectionBarycentricLinesV1V2(
    pmpBarycentricCoordinates line2Start, pmpBarycentricCoordinates line2End, pmpBarycentricCoordinates& intersectionPoint
)
    {
    double intersectionScale1, intersectionScale2, denominator;
...
bool intersectionBarycentricLinesV2V3(
    pmpBarycentricCoordinates line2Start, pmpBarycentricCoordinates line2End, pmpBarycentricCoordinates& intersectionPoint
)
    {
    double intersectionScale1, intersectionScale2, denominator;
...
bool intersectionBarycentricLinesV3V1(
    pmpBarycentricCoordinates line2Start, pmpBarycentricCoordinates line2End, pmpBarycentricCoordinates& intersectionPoint
)
    {
    double intersectionScale1, intersectionScale2, denominator;
...
bool findTriangleEdgeIntersectionInformation(
    pmpBarycentricCoordinates sourceBarycentricLocation, pmpBarycentricCoordinates targetBarycentricLocation,
    pmpBarycentricCoordinates& intersectionPoint, std::vector<vertexIndex> vertexList, halfedgeIndex previousHalfEdge,
    triangleMesh& surface, std::vector<vertexIndex>& involvedVertex, std::vector<int>& uninvolvedVertex
)
    {
    pmpBarycentricCoordinates iCheck;
    bool answer = false;
    if (!(previousHalfEdge == surface.halfedge(vertexList[0], vertexList[1])
          || previousHalfEdge == surface.halfedge(vertexList[1], vertexList[0])))
        {
    ...
    if (!(previousHalfEdge == surface.halfedge(vertexList[1], vertexList[2])
          || previousHalfEdge == surface.halfedge(vertexList[2], vertexList[1])))
        {
    ...
    if (!(previousHalfEdge == surface.halfedge(vertexList[2], vertexList[0])
          || previousHalfEdge == surface.halfedge(vertexList[0], vertexList[2])))
        {
...
*/
void convertBarycentricCoordinates(
    triangleMesh& mesh1, triangleMesh& mesh2, std::unordered_map<faceIndex, int>& faceMap, smspFaceLocation& locationToConvert
)
    {
...
void computePathDistanceAndTangents(
    surfaceMeshShortestPath* smsp, smspFaceLocation& targetPoint, double& distance, vector3& startPathTangent, vector3& endPathTangent
)
    {
    std::vector<point3> pathPoints;
    shortestPathResult geodesic
        = smsp->shortest_path_points_to_source_points(targetPoint.first, targetPoint.second, std::back_inserter(pathPoints));
    distance = std::get<0>(geodesic);
    // Note that the path goes from the target to source, so if we want to know path tangent at the source for force calculation, we must
    // use the *end* of points[]
    if (distance < 0)
        return;
...
void printPoint(point3 a, bool precise)
    {
    if (precise)
        printf("{%.16g,%.16g,%.16g}", a[0], a[1], a[2]);
    else
...
void printBary(smspBarycentricCoordinates a, bool precise)
    {
    if (precise)
        printf("{%.32g,%.32g,%.32g}", a[0], a[1], a[2]);
    else
        printf("{%f,%f,%f}", a[0], a[1], a[2]);
    };
