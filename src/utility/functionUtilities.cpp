...
double vectorMagnitude(vector3 v) { return sqrt(v.squared_length()); };

vector3 normalize(vector3 v) { return v / vectorMagnitude(v); }
...
point3 rotateAboutAxis(point3 p, std::vector<point3> axis, double angle)
    ...
    double s = sin(angle);
    double c = cos(angle);
    rotatedTargetCoordinates.x = axisVector[0] * dotProduct * (1. - c) + shiftedPoint[0] * c
                                 + (axisVector[1] * shiftedPoint[2] - axisVector[2] * shiftedPoint[1]) * s;
    rotatedTargetCoordinates.y = axisVector[1] * dotProduct * (1. - c) + shiftedPoint[1] * c
                                 + (axisVector[2] * shiftedPoint[0] - axisVector[0] * shiftedPoint[2]) * s;
    rotatedTargetCoordinates.z = axisVector[2] * dotProduct * (1. - c) + shiftedPoint[2] * c
                                 + (axisVector[0] * shiftedPoint[1] - axisVector[1] * shiftedPoint[0]) * s;
