...
void hyperRectangularCellList::setGridSize(double _minimumGridSize)
    ...
    if (totalCells == 1)
        printf("You have chosen a grid size that results in a cell list with only one cell. This will be functional, but slower than not "
               "using a grid at all.\n");
    cellListIndexer = Index2D(nMax, totalCells);
    cellIndexer = Index3D(cellNumbers.x, cellNumbers.y, cellNumbers.z);

    // printf("(%f %f %f) (%f %f %f)\n", minimumPositions[0], minimumPositions[1], minimumPositions[2],  maximumPositions[0],
    // maximumPositions[1], maximumPositions[2]); printf("gridSize %f, numbers = (%i %i %i), sizes = (%f %f %f) \n", currentMinimumGridSize,
    // cellNumbers.x,cellNumbers.y,cellNumbers.z, cellSizes.x,cellSizes.y,cellSizes.z);
...
void hyperRectangularCellList::getCellNeighbors(int cellIndex, std::vector<int>& cellNeighbors)
    ...
    if (!periodicSpace)
        ...
        for (int xx = xMin; xx < xMax; ++xx)
            for (int yy = yMin; yy < yMax; ++yy)
                for (int zz = zMin; zz < zMax; ++zz)
                    cellNeighbors.push_back(cellIndexer(wrap(xx, cellNumbers.x), wrap(xx, cellNumbers.y), wrap(xx, cellNumbers.z)));
        };
    };
