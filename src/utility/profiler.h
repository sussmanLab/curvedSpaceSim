...
public:
    ...
    profiler(string profilerName) : name(profilerName)
        {
        functionCalls = 0;
        timeTaken = 0;
        };
    ...
    void print()
        {
        cout << "profiler \"" << name << "\" took an average of " << timing() << " per call over " << functionCalls
             << " calls...total time = " << timing() * functionCalls << endl;
        }
