...
triangleMesh submesher::constructSubmeshFromFaceSet(
    triangleMesh& mesh, std::unordered_set<faceIndex>& faces, std::unordered_map<vertexIndex, int>& vertexMap,
    ...
    // get a mapping between 0-indexed vertices we'll add and the corresponding vertex index ordering in the faces of the full mesh. Add the
    // vertices whie we're at it
    int ii = 0;
    ...
    for (faceIndex currentFace : faces)
        {
        getVertexIndicesFromFace(mesh, currentFace, vidx);
        submesh.add_face((vertexIndex)vertexMap[vidx[0]], (vertexIndex)vertexMap[vidx[1]], (vertexIndex)vertexMap[vidx[2]]);
...
triangleMesh submesher::constructSubmeshFromSourceAndTargets(
    triangleMesh& mesh, faceLocation& source, std::vector<faceLocation>& targets, double& maximumDistanceFromSource,
    std::unordered_map<vertexIndex, int>& vertexMap, std::unordered_map<faceIndex, int>& faceMap
)
    {
    faceMap.clear();
    vertexMap.clear();
    ...
    // first, add to the list of goal faces all faces which contain a target point (and aren't the sourceFace). return early if all of the
    // targets are  in the source face or in the source face and immediately adjacent faces
    ...
    for (halfedgeIndex hi : halfedges_around_face(hf, mesh))
        {
        if (mesh.is_border(edgeIndex(hi)))
            continue; // border edges cannot have adjacent faces to add
    ...
    */
    ...
    while (!explorationStack.empty())
        ...
        for (halfedgeIndex hi : halfedges_around_face(hf, mesh))
            {
            if (mesh.is_border(edgeIndex(hi)))
                continue; // border edges cannot as a rule have adjacent faces we need to add
            neighboringFace = mesh.face(mesh.opposite(hi));
