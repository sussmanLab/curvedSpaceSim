...
public:
    ...
    template<typename T> void addHeaderData(std::string name, const std::vector<T>& data);
    ...
    template<typename T> void registerExtendableDataset(std::string name, int maximumSizePerRecord);

    //! Take a vector of data and append it as a new row in the named dataset
    template<typename T> void extendDataset(std::string name, std::vector<T>& data);
    ...
    template<typename T> void readDataset(std::string name, std::vector<T>& readData, int record = -1);

    //! Return the data type to save a type as (e.g., H5T_NATIVE_INT for ints). Explicit instantiation in the cpp file
    template<typename T> hid_t getDatatypeFor();
