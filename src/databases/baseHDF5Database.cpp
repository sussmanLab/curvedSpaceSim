...
baseHDF5Database::~baseHDF5Database() { H5Fclose(hdf5File); };

template<typename T> void baseHDF5Database::addHeaderData(std::string name, const std::vector<T>& data)
    {
    if (mode == fileMode::readonly)
    ...
    h5dataSpaceCreate dataset(
        hdf5File, name.c_str(), getDatatypeFor<T>(), fileSpace.internalId, H5P_DEFAULT, plist.internalId, H5P_DEFAULT
    );
...
template<typename T> void baseHDF5Database::extendDataset(std::string name, std::vector<T>& data)
    {
    if (mode == fileMode::readonly)
        ERRORERROR("don't write on a readonly file");
    h5dataSpaceOpen dataset(hdf5File, name.c_str(), H5P_DEFAULT);
...
template<typename T> void baseHDF5Database::registerExtendableDataset(std::string name, int maximimumSizePerRecord)
    {
    if (mode == fileMode::readonly)
        ERRORERROR("don't write on a readonly file");
    ...
    // Create the dataset
    h5dataSpaceCreate dataset(
        hdf5File, name.c_str(), getDatatypeFor<T>(), fileSpace.internalId, H5P_DEFAULT, propertyList.internalId, H5P_DEFAULT
    );
    };

template<typename T> void baseHDF5Database::readDataset(std::string name, std::vector<T>& data, int record)
    {
    h5dataSpaceOpen dataset(hdf5File, name.c_str(), H5P_DEFAULT);
    hid_t dataspace = H5Dget_space(dataset.internalId);
    // get the current dimensions of the dataset
    ...
    if (data.size() != dims[1])
        {
        data.resize(dims[1]);
        logMessage(logger::warning, "modelDatabase initialized");
        }
