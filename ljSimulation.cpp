...
int main(int argc, char*argv[])
   ...
   //void setFIREParameters(int _maximumIterations,double _deltaT, double _alphaStart, double _deltaTMax, double _deltaTMin, double _deltaTInc, double _deltaTDec, double _alphaDec, int _nMin, double _forceCutoff, double _alphaMin = 0.75);
    ...
    simpleModelDatabase saveState(N,"./ljOutputDatabase_" + to_string(N) +"_af" + minimizerName+to_string(areaFraction)+".nc",NcFile::Replace, true, false, true);
    ...
    */ //comment here stop 
	...
	double energy; 
	if (ii < switchToFIRE) energy = pairwiseForce->computeEnergy();
	if (ii >= switchToFIRE) energy = LJForce->computeEnergy(); 
