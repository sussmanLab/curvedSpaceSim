...
int main(int argc, char*argv[])
	...
	configuration->fillEuclideanLocations();
        string mposFilename = "../bulk_silo_data/FIRE_"+to_string(N)+"_voronois/" + inputMeshName + to_string(N) + "_voronoi_positions"+to_string(annealStep)+".csv";
    ...
    meshSpace->distance(originMeshPos, configuration->positions, allDists, startTangents,endTangents);
    ...
    plotPointsFile.open("../bulk_silo_data/FIRE_omegaR_Rdivd" + to_string(N) + ".csv", ios::app);
    if (!plotPointsFile)
        {
        plotPointsFile = ofstream("../bulk_silo_data/FIRE_omegaR_Rdivd.csv"); 
        }
    plotPointsFile << omega << ", " <<omegaR << ", " << RoverD << ", " << "\n";  
