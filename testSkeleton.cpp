...
int main(int argc, char*argv[])
    {
    // using Daniel's command line parsing setup because I have never used TCLAP -- 
    // from what I understand, cmd sets the command line itself, the switch args indicate 
    // -(char) commands that can be used to pass specific variables to functions, 
    // and parse puts it all together from argc and argv. Then, we set all the variables 
    // necessary for simulations via the "define variables that correspond to..." 
    ...
    /*
    * In Daniel's code (curvedSpaceSimulation.cpp), he instantiates a cell list neighbor structure here.
    * Again, I don't know what that entails, so I am just going to copy and paste -> comment
    */ 
    double minMaxDim = pow((double)N,(1./3.)); // this and following two lines appear to set the range of available space
    std::vector<double> minPos(3,-minMaxDim);
    std::vector<double> maxPos(3,minMaxDim);

    if(programBranch >= 0) // this statement tells us whether we are in a meshSpace or not. If we are, then we need to use vertices as min/max positions
        {
    ...
    // again -- Daniel's random particle configurations.  
    ...
    // Daniel's simulation code to edit for my examples
